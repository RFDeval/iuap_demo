...
public class DemoOrderBpmService extends GenericBpmService<DemoOrder>{
    ...
    public BPMFormJSON buildBPMFormJSON(String processDefineCode, DemoOrder entity) {
        try {
            ...
            //bpmform.setServiceClass("/order/demo_order/bpmcallback");// 流程审批后，执行的业务处理类(controller对应URI前缀)
            bpmform.setServiceClass("/order/demo_order");
            //设置单据打开类型 uui/react
